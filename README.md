...
compile (rc=65). `build` also takes `--libs-only` (build the library targets and
stop) and `--keep-ir` (leave the generated `<output>.dbg.ll` / `.dbg.o` next to
the target instead of staging them in `/tmp` and deleting them — pair with `-O0`
to get a binary lldb can walk).
