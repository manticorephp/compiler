...
trait EmitLlvmVisit
{
    ...
    public function visitStoreElement(StoreElement $n): string
    {
        ...
        // the slot type is genuinely indeterminate — count it `unchecked`
        // rather than guess. This does not replicate `storeElemBoxesValue`'s
        // KIND_CELL-base / erased-value fallback arms (that predicate is a
    ...
    public function visitStoreDynProp(StoreDynProp_ $n): string
    {
        ...
        // StoreProperty. Never guess it and never skip it silently: count it
        // `unchecked`, ruled the same as every other indeterminate-slot sink.
        $this->checkCellSinkUnchecked();
        return $out;
    }
    ...
    public function visitStoreProperty(StoreProperty $n): string
    {
        ...
        // an undeclared (bag) property, has no single declared type — count
        // those `unchecked` rather than replicate emitStoreProperty's
        // readonly/hook/bag dispatch as a resolver.
        $cls = $n->object->type->kind === \Compile\Mir\Type::KIND_OBJ
            ? ($n->object->type->class ?? '')
            : '';
        $pt = ($cls !== '' && isset($this->classes[$cls]))
