...
// GLOBAL namespace, like every other file in this directory.
//
// These are the DNS WIRE helpers, split out of Dns.php. That file must stay
// in `namespace Runtime\Stdlib` because it declares dns_get_record /
// checkdnsrr / getmxrr, which Zend already defines and would collide with
// during the cold seed. The `__mc_*` helpers have no such conflict — the
// prefix is Manticore-private — and Net.php calls them by their GLOBAL name.
// Left inside the namespace they resolved for nobody: Zend would have
// fatalled on the explicit `\__mc_dns_u16()` calls, Net.php could not see
// them at all, and natively they survived only on the emitter's bare-name
// fallback. The closed-world analyzer reported it as 73 unresolved names,
// which is what made `bin/build` refuse its own preflight.
