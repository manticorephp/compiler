...
function __mc_fileno(\Resource $stream): int
{
    if (\__mc_stream_is_buffered($stream)) {
        // Already an fd (a TLS stream's addr is the fd too), or a memory stream
        // with no fd ($addr 0). Routing it through fileno() would int_to_ptr a
        // small integer and hand libc a bogus FILE*.
        return $stream->addr;
    }
...
{
    if (\__mc_stream_is_buffered($stream)) {
        return false;   // php: cannot truncate a socket/TLS/memory stream
    }
