...
// Keepalive by read timeout: an idle connection is pinged every pingInterval;
// a peer that answers stays connected, a peer that stays silent past
// pongTimeout is dropped (1006, no closing handshake). The server's close
// code is stashed and printed by the client, so the order is fixed.

use function Async\async;
use function Async\spawn;
use Http\WebSocket as WS;

final class Seen { public static int $code = 0; }
    ...
    spawn(function () use ($server) {
        $server->serve(function (\Http\Request $req): \Http\Response {
            return WS\upgrade($req, function (WS\Connection $ws): void {
                ...
                Seen::$code = $ws->closeCode();
            }, (new WS\Options())->pingInterval(0.2)->pongTimeout(0.2));
        });
    });
