...
| A cell-element array retained into a second holder keeps its elements one count too many | `$p->arr = ['x', new D]; $q = clone $p;` then drop both | the element leaks | `__mir_cell_retain`'s ARRAY arm co-owns the elements, `__mir_cell_drop`'s gives them back only at rc 0 — the element-depth disagreement (the http follow-ups item 6) |
